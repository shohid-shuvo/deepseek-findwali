...
import { addOrUpdateFamilyInfo } from '@/utils/api';
...
export default function FamilyInfo({ onNext, onBack }: FamilyInfoProps) {
  ...
  const onSubmit = async (data: FamilyInfoFormData) => {
    try {
      await addOrUpdateFamilyInfo(data);
      onNext();
    } catch (error) {
    ...
    <div>
      ...
      <div className="mt-8 flex justify-between">
        ...
        <button
          ...
          disabled={!isValid}
          className="px-4 py-2 bg-[#ED3284] text-white rounded-md hover:bg-[#d12773] disabled: